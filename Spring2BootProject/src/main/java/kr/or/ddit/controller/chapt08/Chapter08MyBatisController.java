...
public class Chapter08MyBatisController {
	 ...
	 *  		- https://junsker.tistory.com/98 (Oracle 23c 설치 23ai free 버전)
	 *  		- https://junsker.tistory.com/99 (SqlDeveloper 이용하여 Oracle 23c 시스템 계정 접속)
	 *  	2. Oracle SQLDeveloper 설치
	 *  		# 오라클 SqlDeveloper 설치 참고
	 *  		- https://junsker.tistory.com/81 (SqlDeveloper 설치)
	 *  
	 *  	3. 데이터 소스 설정
	 *  		- SpringBoot가 아닌 일반적인 Spring legacy 설정에서는 xml을 활용해서 dataSource를 설정한다.
	 *  			dataSource를 설정하기 위한 여러 가지 자원들이 존재하는데, mybatis 설정, db 접속정보, mapper 위치등을 활용해
	 *  			SqlSessionFactory 빈 생성을 통해 SqlSessionTemplate 이라는 최종 생산품에 해당하는 객체를 만들 수 있다.
	 *  			요청하고자 하는 기능에 따른 쿼리의 결과를 요청할 수 있다.
	 *  			SpringBoot는 이와 같은 설정 자체가 이미 시스템화 되어있다.
	 *  			우리가 별 다른 설정을 하지 않아도 이미 설정이 되어 있기 때문에 설정에 번거로움과 설정에 어려움이 많이 줄었다.
	 *  
	 ...
	 *  		- JPA는 자바 표준 ORM 입니다.
	 *  		- 즉, 구현되어있는 클래스에 테이블을 매핑하기 위한 프레임워크입니다.
	 *  
	 *  		[ # 장점 ]
	 ...
	 *  		
	 *  		***** 패러다임의 불일치 해결
	 *  		객체지향 프로그래밍과 관계형 데이터베이스(RDB) 사이의 구조적인 차이를 극복할 수 있다는 의미로 Java는 객체지향언어이고
	 *  		RDB는 관계형 모델을 사용하는데 객체지향 언어는 객체 구조로 구성되어 있는데 반면 관계형 DB는 테이블 구조로 구성되어있다.
	 ...
	 *  
	 *  		[ # Entity ]
	 *  		- 데이터베이스에서 지속적으로 저장된 데이터를 자바객체에 매핑한 것이다.
	 ...
	 *  		
	 *  		[ # JPA 관련 어노테이션 ]
	 *  		자바빈즈 클래스 객체가 곧 테이블 구조와 같은 형태이기 때문에, VO 클래스 내 @Entity 어노테이션을 통해
	 *  		해당 객체가 테이블과 같은 Entity임을 설정하고 @Table 어노테이션으로 테이블명을 설정해주면 
	 *  		VO클래스는 데이터베이스의 생성되어 있는 케이블과 연결되기 위한 준비가 된다.
	 *  
	 *  		ex)
	 *  		@Entity
	 *  		@Table(name="board")
	 *  		public class Board{...}
	 *  
	 *  		그리고 각 필드(멤버변수)에 @Column(name="board_no")어노테이션으로 테이블 커럼과 매핑 설정을 한다.
	 *  		설정된 Entity 들을 이용하여 JPA 객체를 통해서 함수를 호출하면 (persist, find 등등) 호출된 함수를 통해Spring Data JPA가
	 ...
	 *  		수정해야 하는 번거로움이 발생하지만 JPA 필드만 추가 후, 테이블 정보와 매핑만 시켜주며 JPA가 자동으로 분석을 진행한다.
	 *  		그리고, 사용하는 데이터베이스 종류가 변경된다 하더라도 문제가 없다. 
	 *  		(JPA는 SQL을 직접 건들이지 않고 함수와 같은 기능들로만 쿼리를 요청하고 데이터베이스와 통신하기 때문)
	 *  
	 *  		JPA는 JPA구현체인 hibernate를 이용하여 사용합니다. 
	 *  		그러기 위해서는 관련 의존관계를 등록 후 사용해야 합니다.
	 *  		SQL문을 이용하여 데이터베이스를 연동하여 데이터를 가공하는게 아닌, Methpod를 통해 데이터베이스를 조작할 수 있다는 장점이 있어
	 *  		객체 모델을 이용하여 비지니스 로직을 구성하는데에만 집중을 할 수 있습니다. (쿼리를 직접 구성하지 않기 때문이죠.)
	 *  
	 *  		하지만, *** 프로젝트의 규모와 복잡도가 클 때는 설계가 잘 못 되는 경우 해당 프로젝트의 품질이 떨어진다는 문제가 있습니다.
	 ...
	 *  			- 저는 ibatis를 사용했습니다. 저는 mybatis를 사용했습니다 같은 대답을 하면 됩니다.
	 *  
	 *  	5. 마이바티스란?
	 *  		
	 *  		마이바티스는 자바 퍼시스턴스 프레임워크의 하나로 XML 서술자나 어노테이션을 사용하여 저장 프로시저나 SQL문으로 객체들을 연결시킨다.
	 *  		마이바티스는 Apache 라이센스 2.0 으로 배포되는 자유 소프트웨어이다.
	 *  
	 *  		1) 마이바티스를 사용함으로써 얻을 수 있는 장점
	 *  			- SQL의 체계적인 관리
	 ...
	 *  		3) 마이바티스 플러그인 설정
	 *  			# mybatipse 설정
	 *  			- Help > Eclipse MarkcetPlace > mybatipse 검색 > install
	 *   
	 *  		4) 관련 테이블 생성
	 *  			4-1) board 테이블 생성
	 *  			4-2) member 테이블 생성
	 ...
	 *   			- 게시판 목록 기능 Mapper 인터페이스 메서드 만들기
	 *   			- 게시판 목록 기능 Mapper xml 쿼리 만들기
	 *   			- 게시판 목록 화면 페이지 만들기  ( crud/list.jsp )
	 *   			- 여기까지 확인
	 *   
	 *   			- 게시판 상세화면 컨트롤러 메소드 만들기 ( crudRead:get )
	 *   			- 게시판 상세화면 서비스 인터페이스 메소드 만들기
	 *   			- 게시판 상세화면 서비스 클래스 메소드 만들기
	 *   			- 게시판 상세화면 Mapper 인터페이스 메서드 만들기
	 *   			- 게시판 상세화면 Mapper xml 쿼리 만들기
	 *   			- 게시판 상세화면 페이지 만들기  ( crud/read.jsp )
	 *   			- 여기까지 확인
	 *   
	 *   			- 게시판 수정화면 컨트롤러 메소드 만들기 ( crudModify:get )
	 *   			- 게시판 수정화면 서비스 인터페이스 메소드 만들기
	 *   			- 게시판 수정화면 서비스 클래스 메소드 만들기
	 *   			- 게시판 수정화면 Mapper 인터페이스 메서드 만들기
	 *   			- 게시판 수정화면 Mapper xml 쿼리 만들기
	 *   			- 게시판 수정화면 페이지 만들기  ( crud/register.jsp )
	 *   
	 ...
	 *   			- 게시판 검색 기능 Mapeer 메서드 추가
	 *   			- 게시판 검색 기능 Mapeer xml 쿼리 추가
	 *   			- 여기까지 확인
	 *   
	 *   			- 기본 CRUD 끝
	 *   
	 ...
	 *   			- mybatis.type-aliases-package=kt.or.ddit.vo
	 *   
	 *   			# TypeAlias 설정 방법
	 ...
	 *   
	 *   		1) 마이바티스 설정
	 *   			1-1) 매핑파일 수정(board_Mapper.xml)
	 *   			- create 부분의 속성 추가
	 *   			> <insert id="create" parameterType="Board" useGeneratedKeys="true">
	 *   			> <selectKey order="BEFORE" resultType="int" keyProperties="boardNo">
	 ...
	 *   			> </insert>
	 *   			
	 *   			아래 insert 쿼리가 실행되기 전, selectKey 태그 내에 있는 쿼리가 먼저 실행되어
	 *  			최신의 boardNo를 생성하고 생성된 boardNo를 sql쿼리가 담겨있는 insert 태그까지 넘어올 때 
	 *  			넘겨주고 있는 파라미터 (board)의 property인 boardNo에 세팅한다.
	 *  			세팅된 boardNo를 아래에서 insert시, 등록 값으로 사용하고 board 라는 자바빈즈 객체에 담겨 최종 컨트롤러 까지 넘어간다.
	 *  			
	 *  			*** seq_board.nextval 대신 현재의 seq 번호를 가져오기 위해 currval를 사용하게 되는 경우가 있습니다.
	 *  				이때 사용시 주의사항이 있습니다.
	 *  
	 *  				- select seq_board.currval from dual
	 *					> 위 select 쿼리를 사용시, currva를 사용하는데 있어서 사용불가에 대한 여러 가지 에러가 발생할 수 있습니다.
	 *						currva를 사용할 때는 select seq_board.nextval from dual와 같이 먼저 최초로 실행한 후,
	 ...
	 *					쿼리로 가져오면 된다. (권고사항 아님)
	 *
	 *				1-2) 컨트롤러 메서드 에서 srudRegister 부분의 등록이 되고나서 리턴되는 방향성을 수정
	 *				- 지금은 완료페이지로 전환되지만, board에 담겨있는 boardNo를 이용하여 상세보기 페이지로 전환해도 무방하다!		
	 *   
	 *   	10. 동적 SQL
	 ...
	 *   		- 여기 까지 확인
	 *   
	 *   		- 회원 등록 기능 컨트롤러 메서드 만들기 (crudmemberRegisterForm:post)
	 *   		- 회원 등록 기능 인터페이스 메서드 만들기
	 *   		- 회원 등록 기능 클래스 메서드 만들기
	 *   		- 회원 등록 Mapeer 인터페이스 메서드 만들기
	 *   		- 회원 등록 Mapeer xml 쿼리 추가   
