...
public class Chapter04Controller {
	...
	// 반환값이 /sub/goHome0202 이므로 뷰(/sub/home0202.jsp)를 가리킨다.
	...
	// '/'로 시작하면 웹 애플리케이션의 컨텍스트 경로를 포함한 절대 경로를 의미한다.
	// 브라우저 URL에서 포함된 컨텍스트의 개념이 아니라, jsp 페이지를 찾는 개념에서의 컨텍스츠를 의미합니다.
	// '프로젝트명/chapt04/sub/home0204'를 찾으려 할 것입니다.
	// 이때, forword 의 형태로 찾는데 프레임워느는 컨트롤러가 응답하는 문자열의 형태가 resolver를 통해서 처리되므로
	// suffix, prefix 정보를 넣어 파일 위치의 절대 경로로 jsp를 찾아 응답하려고 할 것 입니다.
	 ...
	 * 	- @RedponseBody를 지정하지 않으면 HTTP 404 에러가 발생한다.
