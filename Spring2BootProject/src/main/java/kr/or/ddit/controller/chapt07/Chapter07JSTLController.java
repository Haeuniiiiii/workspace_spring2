...
public class Chapter07JSTLController {
	 ...
	 * 		<c:otherwise>	| <c:when> 요소에서 지정한 조건에 모두 일치하지 않을 떄 처리할 내용을 구현한다. (<c:choose> 요소에서 사용)
	 * 
	 * 		<c:forEach>		| 컬렉션이나 배열의 각 항목을 처리하거나 일반적인 반복문을 처리할 때 사용한다.
	 * 		<c:forToken>	| 구분자로 구분된 각각의 토큰을 처리할 때 사용한다.
	 * 		<c:import>		| URL을 사용하여 다른 자원을 삽입힌다.
	 * 		<c:url>			| URL을 재작성한다.
	 ...
	 * 
	 * 		[ taglib 지시자 사용 ::: <%@ taglib uri="jakarta.tag.fmt" prefix="fmt" %>
	 * 
	 * 
	 * 		3) function 태그 라이브러리
	 * 		요소 						| 설명
	 ...
	 * 
	 * 		4. 코어 태그
	 * 		- 조건 분기나 반복처리 그리고 변수의 지정 등과 같이 논리적인 처리를 위해 사용되는 
	 * 			스크립트 코드를 대체하기 위한 태그를 제공합니다.
	 * 
	 * 		1) <c:out>
	 * 		- JSPWriter 에 값을 적절하게 처리한 후에 출력한다.
	 * 		속성			|	타입		|	설명
	 * 		────────────────────────────────────────────────────────────────────────────────────────────────────────────────
	 * 		value		| Object	| 출력할 값
	 * 		escapeXml	| boolean	| 특수 문자를 변환할지의 여부(true 시, xml / html 태그를 이스케이프처리)
	 * 		default		| Object	| value 의 결과값이 null 인 경우 출력할 값 (대신 출력됨)
	 * 		────────────────────────────────────────────────────────────────────────────────────────────────────────────────
	 * 
	 * 		2) <c:set>
	 * 		- JSP 에서 사용할 변수를 설정한다.
	 ...
	 * 
	 * 		8) <c:otherwise>
	 * 		- <c:when> 요소에서 지정한 조거에 모두 일치하지 않을 때 처리할 내용을 구현한다.  
	 * 		- <c:choose> 요소에서 사용한다.
	 * 
	 * 		9) <c:forEach>
	 ...
	 * 		charEncoding| String	| 입력 리소스의 쿤텐츠 문자 인코딩
	 * 		context		| String	| 다른 웹 어플리케이션의 컨텍스트 지정
	 * 		────────────────────────────────────────────────────────────────────────────────────────────────────────────────
	 * 
	 * 		12) <c:url>
	 * 		- URL을 재작성한다.
