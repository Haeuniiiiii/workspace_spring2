...
public class Chapter07JSPCintroller {
	/*
	 * [ 7장. JSP ]
	 * 	1. 커스텀 태그 라이브러리
	 * 		- 스크립트 요소가 많아지면 많아질수록 JSP 코드는 복잡해집니다.
	 * 			이 문제를 해결하는 한 가지 방법은 커스텀 태그를 사용하는 것입니다.
	 * 			이 커스텀 태그를 사용하면 뷰를 표시하기 위한 로직을 공통화하거나,
	 * 			표현하기 복잡한 로직을 캡슐화 할 수 있어서 JSP의 구현코드를 간결하게 만들 수 있습니다.
	 * 			그리고 커스텀 태그를 모아놓은 것을 커스텀 태그 라이브러리 라고 합니다.
	 * 
	 * 		# 대표적인 태그 라이브러리
	 ...
	 * 
	 * 	2. 표현언어 (EL)
	 * 		- JSP는 EL (Expression Language)이라는 표현언어를 사용해 값의 참조, 출력, 연산을 할 수 있다.
	 * 		- EL 표현식은 ${...} 또는 #{...} 형식으로 작성합니다.
	 * 
	 * 			# EL을 사용하여 객체를 조회하는 방법은 다음과 같습니다.
	 * 			- 자바빈즈 프로퍼티를 조회하는 경우, "속성명.프로퍼티명"을 지정합니다. (실제 프로퍼티가 아닌 getter 를 기반으로 설정됨)
	 * 			- List 나 배열 요소를 조회하는 경우 "속성명[요소위치]"를 지정한다.
	 * 			- Map 요소를 조회하는 경우, "속성명.프로퍼티명" 또는 "속성명[프로퍼티명]"을 지정한다.
	 * 
	 *  		# 사용 가능한 연산자
	 *  		- EL 에서는 다음과 같은 연산자를 사용할 수 있다.
	 *  
	 *  		# 산술연산자
	 * 		 		+ 		│	-		│	*		│	/(div)	│	%(mood)
	 *  		─────────────────────────────────────────────────────────────────────────────
	 ...
	 *  		|| (or)	| 두 피연산자 중 하나 또는 모두 true 이면 bool true를 반환하고, 그렇지 않으면 false를 반환한다.
