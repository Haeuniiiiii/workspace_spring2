...
@Controller
@RequestMapping("/item")
public class Chpater09FileUploadController {
	 ...
	 *			- 스프링부트 프로젝트 초기 설정 시, 해당 dependency에 포함
	 ...
	 *			- spring.servlet.multipart.max-file-size-threshould=12MB
	 *
	 *		# 파일 업로드 경로 설정
	 ...
	 * 			- 파일업로드 등록 화면 컨트롤러 메서드 만들기 (itemRegisterForm:get)
	 ...
	 * 			- 여기까지 확인
	 * 
	 * 			- 파일업로드 목록 화면 컨트롤러 메서드 만들기 (itemList:get)
	 ...
	 * 			- 여기까지 확인
	 * 
	 * 			- 파일업로드 수정 화면 컨트롤러 메서드 만들기 (itemModifyForm:get)
	 ...
	 * 			- 여기까지 확인
	 * 
	 * 			- 파일업로드 수정 기능 컨트롤러 메서드 만들기 (itemModify:post)
	 ...
	 * 			- 여기까지 확인
	 * 
	 * 			- 파일업로드 삭제 화면 컨트롤러 메서드 만들기 (itemRemoveForm:get)
	 ...
	 * 			- 여기까지 확인
	 * 
	 * 			- 파일업로드 삭제 기능 컨트롤러 메서드 만들기 (itemRemove:post)
	...
	// 업로드 하기 위한 경로 설정
	// [ 방법 1 ] application.properties 파일에 내장된 경로를 설정할 수 있다. 
	@Value("${kr.or.ddit.upload.path}")
	private String localPath;
	
	// [ 방법 2 ] 파일 업로드 설정 경로
//	private String localPath = "C:/upload";
	...
	public String itemManual() {
		log.info("itemManual() 실행...!!");
		return "chapt09/item/manual";
	}
	...
	public String itemRegister(Item item, Model model) throws Exception {
		...
		// 넘겨받은 파일을 이용하여 파일 업로드 (복사)를 진행한다. (발생하는 예외는 throws Exception)
		// return : UUID + '_' + 원본파일명
	...
	@GetMapping("/modify")
	public String itemModifyForm(int itemId, Model model) { //쿼리스트링 itemId
	...
	@PostMapping("/modify")
	public String itemModify(Item item, Model model) throws Exception { //Item객체 필요
		MultipartFile file = item.getPicture();
	...
	@GetMapping("/remove")
	public String itemRemoveForm(int itemId, Model model) { //쿼리스트링 itemId
		Item item = itemService.read(itemId);
		model.addAttribute("item", item);
	...
	@PostMapping("/remove")
	public String itemRemove(int itemId, Model model) { //쿼리스트링 itemId
