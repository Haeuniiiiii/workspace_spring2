...
<body>
	...
	<p>1) URL 경로 상의 쿼리 파라미터 정보로 부터 요청 데이터를 취득할 수 있다. </p>
	<a href="/chapt05/register?userId=hongkd&password=1234">요청(쿼리파라미터)</a>
	
	<p>2) HTML Form 필드명과 컨트롤러 매개변수 명이 일치하면 요청 데이터를 취득할 수 있다.</p>
	...
	<p>1) URL 경로 상의 변수가 여러개일때 @PathVariable어노테이션을 사용하여 특정한 경로 변수명을 지정해준다. </p>
	<a href="/chapt05/register/hongkd/1000">요청(경로상의 포함된 파라미터)</a>
	...
	<p>2) 폼 텍스트 필드 요소값을 자바빈즈 매개변수로 기본 데이터 타입인 정수 타입 매개변수로 처리한다.</p>
    ...
    <p>2) 쿼리 파라미터 (dateOfBirth=20250704)로 전달받은 값은 Date 타입으로 데이터를 받을 수 있는가?</p>
    ...
    <p>5) 쿼리 파라미터 (dateOfBirth)로 전달받은 값은 Member 객체를 이용한 Date 타입으로 데이터를 받을 수 있는가?</p>
	...
	<p>3) 폼 라디오 버튼 요소값을 기본 데이터 타입인 문자열 타입 매개변수로 처리한다.</p>
	...
	<p>14) 폼 텍스트 필드 요소값을 중첩된 자바빈즈 매개변수로 처리한다.</p>
